...
void LSTMUnitLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
  ...
  for (int n = 0; n < num; ++n) {
    for (int d = 0; d < hidden_dim_; ++d) {
      ...
      const Dtype g = relu(X[3 * hidden_dim_ + d]);             
      const Dtype c_prev = C_prev[d];
      const Dtype c = f * c_prev + i * g;
      C[d] = c;
      const Dtype relu_c = relu(c);                            
      H[d] = o * relu_c;                                    
    }
    C_prev += hidden_dim_;
    X += x_dim;
  ...
  for (int n = 0; n < num; ++n) {
    for (int d = 0; d < hidden_dim_; ++d) {
      ...
      const Dtype c_term_diff =
          C_diff[d] + H_diff[d] * o * int(c>0) ;               
      *c_prev_diff = c_term_diff * f;
      *i_diff = c_term_diff * g * i * (1 - i);
      *f_diff = c_term_diff * c_prev * f * (1 - f);
      *o_diff = H_diff[d] * relu_c * o * (1 - o);
      *g_diff = c_term_diff * i * int(g>0);                       	  
    }
    C_prev += hidden_dim_;
    X += x_dim;
    C += hidden_dim_;
...
template <typename Dtype>
void LSTMUnitLayer<Dtype>::Backward_eb_cpu(const vector<Blob<Dtype>*>& top,
    const vector<bool>& propagate_down, const vector<Blob<Dtype>*>& bottom) {
  CHECK(!propagate_down[2]) << "Cannot backpropagate to sequence indicators.";
  if (!propagate_down[0] && !propagate_down[1]) { return; }
